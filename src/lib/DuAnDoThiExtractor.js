...
class DuAnExtractor {
	...
	extractOnePage(url, handleDone) {
		osmosis
			.get(url)
			.find('.pjl-item')
			.follow('.pjl-img a@href')
			.set({
				'name'			:'.prj-content .pc-text > h1',
				'diachi'		:'.prj-content .pc-text-address[1] > p',
				'hdLat'		:'.container-default input[id="hddLatitude"]@value',
				'hdLong'	:'.container-default input[id="hddLongtitude"]@value',
				'duAnID'    :'#form1 :source'
			})
			.data(function(duan) {
				let adduan = {
