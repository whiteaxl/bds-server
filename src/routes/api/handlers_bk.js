...
internals.getAllAds = function(req, reply) {
	...
	adsModel.queryAll(function(result){
		for (var i = 0; i < result.length; i++) { 
    		var ads = result[i];
    		//console.log(ads)
    		if(result[i].default.place){
    			console.log("1")
    			if(result[i].default.place.geo){
    				console.log("2")
	    			result[i].map={
	    				center: {
							latitude: 	result[i].default.place.geo.lat,
							longitude: 	result[i].default.place.geo.lon
						},
	    				marker: {
							id: i,
							coords: {
							...
							options: {
								labelContent : result[i].default.gia
							},
							data: 'test'
						},
						options:{
